...
/// Blanket implementation so `Arc<T>` can be used wherever `Provider` is expected.
///
/// This delegates every method to the inner `T`, preserving any overrides the
/// concrete type provides. Without this blanket impl, downstream crates cannot
/// write `impl Provider for Arc<MyMock>` due to the orphan rule.
#[async_trait]
impl<T: Provider + ?Sized> Provider for std::sync::Arc<T> {
    ...
    async fn simple_chat(
        ...
        (**self).simple_chat(message, model, temperature).await
    }
    ...
    fn stream_chat_with_system(
        ...
        (**self).stream_chat_with_system(system_prompt, message, model, temperature, options)
    }
