...
response="$(curl -fsS -X POST "${base_url}/canary/audit/archive" \
  -H "x-zeroclaw-drill-token: ${drill_token}" \
  -H "content-type: application/json" \
  --data "${payload}")"

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
archive_dir="${output_root%/}/canary-audit-archive-${timestamp}"
mkdir -p "${archive_dir}"
printf '%s\n' "${response}" > "${archive_dir}/archive.json"

jq '{archived_records:(.records|length),deleted,remaining}' "${archive_dir}/archive.json" > "${archive_dir}/summary.json"

echo "audit_archive_file=${archive_dir}/archive.json"
echo "audit_archive_summary=${archive_dir}/summary.json"
